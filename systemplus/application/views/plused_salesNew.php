...
?>		
		...
		<section id="content" class="container_12 clearfix" data-sort=true>
			<h1 class="grid_12 margin-top no-margin-top-phone">Sales</h1>
			<div class="row">
					...
					<div class="grid_3">
						<div class="box">
							...
							<div class="content" style="margin:10px;text-align:center;height:68px;">
								...
								<input type="button" name="inviaOPerc" id="inviaOPerc" value="Retrieve Data Perc(test)" />
							</div>
						</div>
					</div>						
				</form>
	...
	$('#inviaO').on('click', function(e){
        ...
        $('<div/>', {'class':'myDlgClass', 'style' :'background:#fff url(http://plus-ed.com/vision_ag/img/logo_loading.gif) center center no-repeat;', 'id':'link-'+($(this).index()+1)})
            .html($('<iframe/>', {
                'src' : "http://plus-ed.com/vision_ag/index.php/backoffice/salesDetailNew/?"+passingData,
	...
	$('#inviaOPerc').on('click', function(e){
		var centriChecked = $('input.chCentri:checked').length;
		var accomoChecked = $('input.chAcco:checked').length;
		if(centriChecked==0){
			alert("Please, select one or more campus!");
        ...
        $('<div/>', {'class':'myDlgClass', 'style' :'background:#fff url(http://plus-ed.com/vision_ag/img/logo_loading.gif) center center no-repeat;', 'id':'link-'+($(this).index()+1)})
            .html($('<iframe/>', {
                'src' : "http://plus-ed.com/vision_ag/index.php/backoffice/salesDetailNewPerc/?"+passingData,
                'style' :'width:100%; height:100%;border:none;',
            })).appendTo('body')
            .dialog({
                'title' : 'Sales detail',
