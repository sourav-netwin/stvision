...
<style>
    textarea,input[type=text],input[type=file],.chzn-container{
        max-width:255px;
    }
    .error{
...
//            $( "li#bocampus ul.sub li#bocampus_5" ).addClass("current");
		...
		$( "li#myaccount ul.sub" ).css('display','block');	
            ...
            // EXTRA INFORMATION
            $('.starterDecRad label').tipsy({gravity: 's'});
             
             $( "body" ).on( "mouseleave", ".starterDecRad",function(){
                 $('.tipsy').hide();
             });
            ...
            <?php /* REMOVED, IT'S FOR OFFICE USE ONLY ?>
            // append file controls dynamically..
            var num = 0;
            $( "body" ).on( "click", "#btnAddFile",function(){
                buildUploadFileControl('','');
                $("#upload-message").removeClass('tuition_error').removeClass('tuition_success').html('');
            });
            
            function buildUploadFileControl(fileTitle,fileName){
                num++;
                var controlHTML = "";
                controlHTML += "<div id='row_"+num+"' class='row'>";
                controlHTML += "    <label for='txtFile_"+num+"' >";
                controlHTML += "            <strong>Upload file</strong>";
                controlHTML += "    </label>";
                controlHTML += "    <div class='form-data' >";
                controlHTML += "            <input type='text' placeholder='file title' data-num='"+num+"' id='txt_otherFile_"+num+"' name='txt_otherFile_"+num+"' class='fileTitleText'   maxlength='100' value='"+fileTitle+"' />";
                controlHTML += "            <input type='file' disabled='disabled' placeholder='other document "+num+"' name='otherFile_"+num+"' id='otherFile_"+num+"' >";
                controlHTML += "            <img id='loading_otherFile_"+num+"' style='display:none;height: 16px;width:16px;' src='<?php echo base_url();?>img/tuition/throbber.gif'/>";
                controlHTML += "            <input type='hidden' value='"+fileName+"' name='hiddold_otherFile_"+num+"' id='hiddold_otherFile_"+num+"' >";
                controlHTML += "            <div id='error_otherFile_"+num+"' class='error'></div>";
                if(fileName != '')
                    controlHTML += "            <a target='_blank' id='link_otherFile_"+num+"' data-num='"+num+"' href='<?php echo base_url();?>upload/teachers/office_other/"+fileName+"'>"+fileName+"</a>";
                else
                    controlHTML += "            <a target='_blank' id='link_otherFile_"+num+"' data-num='"+num+"' href='javascript:void(0);'></a>";
                controlHTML += "            <img title='Remove file' class='delete-icon-tuition' data-num='"+num+"' src='<?php echo base_url();?>/img/tuition/cross-circle.png' />";
                controlHTML += "    </div>";
                controlHTML += "</div>";
               
                $(".file-controls").append(controlHTML);
            }
            
            // end of dynamic controls
            $( "body" ).on( "keypress", ".fileTitleText",function(){
                var text = $(this).val();
                var num = $(this).attr('data-num');
                if(text.length > 0)
                    $("#otherFile_"+num).removeAttr('disabled');
                else
                    $("#otherFile_"+num).attr('disabled','disabled');
            });
            
            $( "body" ).on( "blur", ".fileTitleText",function(){
                var fileTitle = $(this).val();
                var num = $(this).attr('data-num');
                var edit_id = $("#edit_id").val();
                var oldFileName = $("#hiddold_otherFile_"+num).val();
                if(oldFileName != '')
                $.post( "<?php echo base_url();?>index.php/teachers/updatefiletitle",{
                    'edit_id':edit_id,
                    ...
                    {
                        $("#upload-message").removeClass('tuition_error').addClass('tuition_success').html(data.message);
                    }else
                    {
                        $("#upload-message").removeClass('tuition_success').addClass('tuition_error').html(data.message);
                    }
                },'json'); 
            });
            
            $( "body" ).on( "click", ".delete-icon-tuition",function(){
                var num = $(this).attr('data-num');
                var edit_id = $("#edit_id").val();
                var oldFileName = $("#hiddold_otherFile_"+num).val();
                if(confirm("Are you sure to remove this file?"))
                {
                    $.post( "<?php echo base_url();?>index.php/teachers/deletefile",{
                        'edit_id':edit_id,
                        'oldFileName':oldFileName
                    }, function( data ) {
                        if(parseInt(data.result))
                ...
                $( "body" ).on( "blur", "input[type=file]", prepareUpload);

                // Grab the files and set them to our variable
                function prepareUpload(event)
                {
                    var thisfile = $(this).attr('id');
                    if(thisfile != 'cvFile' && thisfile != 'otherFile'){
                        var fileTitle = $("#txt_" + thisfile).val();
                        if(fileTitle == '')
                            $("#upload-message").removeClass('tuition_error').addClass('tuition_success').html('Please enter title for uploading file first.');
                        else
                        {
                            files = event.target.files;
                            uploadFiles(event,thisfile);
                        }
                    }
                }
                ...
                function uploadFiles(event,thisfile)
                {
                        event.stopPropagation(); // Stop stuff happening
                        event.preventDefault(); // Totally stop stuff happening

                        // START A LOADING SPINNER HERE
                        $("#loading_"+thisfile).show();
                        ...
                        $.ajax({
                            url: '<?php echo base_url();?>index.php/teachers/ajaxfileupload/files',
                            type: 'POST',
                            data: data,
                            cache: false,
                            dataType: 'json',
                            ...
                            success: function(data, textStatus, jqXHR)
                            {
                                if(typeof data.error === 'undefined')
                                {
                                    ...
                                    $("#error_"+thisfile).html('');
                                    submitForm(event, data,thisfile);
                                }
                                else
                                {
                                    // Handle errors here
                                    $("#error_"+thisfile).html(data.error);
                                    console.log('ERRORS: ' + data.error);
                                    $("#loading_"+thisfile).hide();
                                }
                            },
                ...
                function submitForm(event, data,thisfile)
                {
                    ...
                    var titleText = $("#txt_"+thisfile).val();
                    var edit_id = $("#edit_id").val();
                    var oldFileName = $("#hiddold_"+thisfile).val();
                    formData = formData + '&title=' + titleText;
                    formData = formData + '&edit_id=' + edit_id;
                    formData = formData + '&oldfilename=' + oldFileName;

                    $.ajax({
                        url: '<?php echo base_url();?>index.php/teachers/ajaxfileupload',
                        type: 'POST',
                        data: formData,
                        cache: false,
                        dataType: 'json',
                        success: function(data, textStatus, jqXHR)
                        {
                            ...
                            {
                                // Success so call function to process the form
                                $("#hiddold_"+thisfile).val(data.file_name);
                                $("#link_"+thisfile).attr('href','<?php echo base_url();?>upload/teachers/office_other/'+data.file_name);
                                $("#link_"+thisfile).html(data.file_name);
                            }
                            else
                            {
                                // Handle errors here
                ...
                // generate existing files controls
                <?php if($appOtherFiles){
                        foreach($appOtherFiles as $otherFildRec){
                            ?>buildUploadFileControl('<?php echo $otherFildRec['tof_title'];?>','<?php echo $otherFildRec['tof_filename'];?>');<?php 
                        }
                    }
                ?>
               // $('#txtFirstName').tipsy();
                    ...
                    <form enctype="multipart/form-data" method="post" class="no_validate" id="frmTeacher" action="">
                        <div class="row">
                                ...
                                <div class="form-data" style="margin-left: 130px;">
                                        <input  type="text" id="txtFirstName" name="txtFirstName" class="required"   maxlength="100" value="<?php echo set_value('txtFirstName',  $formData['ta_firstname']);?>" >
                        ...
                        <div class="row">
                                ...
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" id="txtLastName" name="txtLastName" class="required"   maxlength="100" value="<?php echo set_value('txtLastName',  $formData['ta_lastname']);?>" >
                                        <div class="error"><?php echo form_error('txtLastName');?></div>
                                </div>
                        </div>
                        
                        <div class="row">
                                <label for="txtDateofBirth" style="width: 115px;">
                                        <strong>Birth date</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" placeholder="dd/mm/yyyy" id="txtDateofBirth" name="txtDateofBirth" class="required"   maxlength="100" value="<?php echo set_value('txtDateofBirth',  $formData['ta_date_of_birth']);?>" >
                                        <div class="error"><?php echo form_error('txtDateofBirth');?></div>
                                </div>
                        </div>
                        <div class="row">
                                <label for="selNationality" style="width: 115px;">
                        ...
                        <div class="row">
                                ...
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" placeholder="E-mail address" name="txtEmail" id="txtEmail" class="required"   maxlength="100" value="<?php echo set_value('txtEmail',  $formData['ta_email']);?>" >
                                        <div class="error"><?php echo form_error('txtEmail');?></div>
                                </div>
                        </div>
                        <div class="row">
                                <label for="txtTelephone" style="width: 115px;">
                                        <strong>Telephone</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" class="required" placeholder="Telephone" id="txtTelephone" name="txtTelephone" value="<?php echo set_value('txtTelephone',$formData['ta_telephone']);?>" >
                        ...
                        <div class="row">
                                <label for="txtAddress" style="width: 115px;">
                                        <strong>Address</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                        <textarea rows="3" class="required" placeholder="Address" id="txtAddress" name="txtAddress"><?php echo set_value('txtAddress',$formData['ta_address']);?></textarea>
                                        <div class="error"><?php echo form_error('txtAddress');?></div>
                                </div>
                        </div>
                        <div class="row">
                                <label for="txtCity" style="width: 115px;">
                                        <strong>City</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" class="required" placeholder="City" id="txtCity" name="txtCity" value="<?php echo set_value('txtCity',$formData['ta_city']);?>" >
                        ...
                        <div class="row">
                                <label for="selCountry" style="width: 115px;">
                        ...
                        <div class="row">
                                <label for="txtNiNumber" style="width: 115px;">
                                        <strong>NI Number</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" placeholder="NI Number" name="txtNiNumber" id="txtNiNumber" class="required"   maxlength="100" value="<?php echo set_value('txtNiNumber',  $formData['ta_ni_number']);?>" >
                        ...
                        <div class="row">
                                ...
                                <div class="form-data" style="margin-left: 130px;">
                                    <label for="radRtwinukYes" style="margin-right:15px;">
                                        <input <?php echo ($formData['ta_right_to_work_uk'] == '1' ? 'checked="checked"' : '');?> type="radio" id="radRtwinukYes" name="radRtwinuk" value="1" />
                                        Yes
                                    </label>
                                    <label for="radRtwinukNo">
                                        <input <?php echo ($formData['ta_right_to_work_uk'] == '0' ? 'checked="checked"' : '');?> type="radio" id="radRtwinukNo" name="radRtwinuk" value="0" />
                                        No
                                    </label>
                        ...
                        <div class="row">
                                <label for="txtNiCategory" style="width: 115px;">
                                        <strong>NI Category</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                    <input type="text" placeholder="NI Category" name="txtNiCategory" id="txtNiCategory" class="required" maxlength="100" value="<?php echo set_value('txtNiCategory',  $formData['ta_ni_category']);?>" />
                        ...
                        <div class="row">
                                ...
                                <div class="form-data" style="margin-left: 130px;">
                                    ...
                                    <label for="radSLRYes" style="margin-right:15px;">
                                        <input autocomplete="off" <?php echo ($formData['ta_making_slr'] == '1' ? 'checked="checked"' : '');?> type="radio" id="radSLRYes" name="radSLR" value="1" />
                                        Yes
                                    </label>
                                    <label for="radSLRNo">
                                        <input autocomplete="off" <?php echo ($formData['ta_making_slr'] == '0' ? 'checked="checked"' : '');?> type="radio" id="radSLRNo" name="radSLR" value="0" />
                                        No
                                    </label>
                                    <div class="error"><?php echo form_error('radSLR');?></div>
                                </div>
                        </div>
                        <div id="slrPlanDiv" class="row">
                                <label style="width: 115px;">
                                        <strong>SLR Plan</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                    <strong>If yes please advise if plan 1 or 2</strong><br/>
                                    <label for="radSLRPlan1" style="margin-right:15px;">
                                        <input autocomplete="off" <?php echo ($formData['ta_slr_plan'] == 'Plan 1' ? 'checked="checked"' : '');?> type="radio" id="radSLRPlan1" name="radSLRPlan" value="Plan 1" />
                                        Plan 1
                                    </label>
                                    <label for="radSLRPlan2">
                                        <input autocomplete="off" <?php echo ($formData['ta_slr_plan'] == 'Plan 2' ? 'checked="checked"' : '');?> type="radio" id="radSLRPlan2" name="radSLRPlan" value="Plan 2" />
                                        Plan 2
                                    </label>
                        ...
                        <div class="row">
                                ...
                                <div class="form-data" style="margin-left: 130px;">
                                    ...
                                    <label for="radP45Yes" style="margin-right:15px;">
                                        <input autocomplete="off" <?php echo ($formData['ta_p45_status'] == '1' ? 'checked="checked"' : '');?> type="radio" id="radP45Yes" name="radP45" value="1" />
                                        Yes
                                    </label>
                                    <label for="radP45No">
                                        <input autocomplete="off" <?php echo ($formData['ta_p45_status'] == '0' ? 'checked="checked"' : '');?> type="radio" id="radP45No" name="radP45" value="0" />
                                        No
                                    </label>
                                    <div class="error"><?php echo form_error('radP45');?></div>
                                </div>
                        </div>
                        <div id="strDeclDiv" class="row">
                                <label style="width: 115px;">
                                        <strong>Starter Declaration</strong>
                                </label>
                                <div class="form-data starterDecRad" style="margin-left: 130px;">
                                    <strong>If no(P45) please select 1 starter declaration A/B/C</strong><br/>
                                    <label title="Starter Declaration A: This is my first job since last 6 April and I have not been receiving taxable Jobseeker's Allowance, Employment and Support Allowance, taxable Incapacity Benefit, State or Occupational Pension." for="radStarterDeclarationA" style="margin-right:15px;">
                                    ...
                                    <label title="Starter Declaration B: This is now my only job but since last 6 April I have had another job, or received taxable Jobseeker's Allowance, Employment and Support Allowance or taxable Incapacity Benefit. I do not receive a State or Occupational Pension.
" for="radStarterDeclarationB" style="margin-right:15px;">
                                        ...
                                        Starter Declaration B
                                    </label>
                                    <label title="Starter Declaration C: As well as my new job, I have another job or receive a State or Occupational Pension.
" for="radStarterDeclarationC">
                        ...
                        <div class="row">
                                <label for="txtAblilityFrom" style="width: 115px;">
                                        <strong>Availability from</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" placeholder="dd/mm/yyyy" name="txtAblilityFrom" id="txtAblilityFrom" class="required"   maxlength="100" value="<?php echo set_value('txtAblilityFrom',  $formData['ta_ablility_from']);?>" >
                                        <div class="error"><?php echo form_error('txtAblilityFrom');?></div>
                                </div>
                        </div>
                        <div class="row">
                                <label for="txtAblilityTo" style="width: 115px;">
                                        <strong>Availability to</strong>
                                </label>
                                <div class="form-data" style="margin-left: 130px;">
                                        <input type="text" placeholder="dd/mm/yyyy" name="txtAblilityTo" id="txtAblilityTo" class="required"   maxlength="100" value="<?php echo set_value('txtAblilityTo',  $formData['ta_ablility_to']);?>" >
                        ...
                        <div class="row">
                                ...
                                <div class="form-data" style="margin-left: 130px;">
                                    ...
                                    <br/>
                                    <input type="text" placeholder="If other, please specify" id="txtOtherDiploma" name="txtOtherDiploma" value="<?php echo set_value('txtOtherDiploma',  $formData['ta_other_diploma']);?>">
                                    <div class="error"><?php echo form_error('txtOtherDiploma');?></div>
                                </div>
                        </div>
                        <div class="row">
                                <label for="cvFile" style="width: 115px;">
                        ...
                        <div class="row">
                                <label for="otherFile" style="width: 115px;">
                        ...
                        <div class="row">
                                <label for="passportOrIdCard" style="width: 115px;">
                        ...
                        <div class="row">
                                <div class="form-data" style="margin-left: 130px;">
                                    ...
                                    <input class="btn btn-tuition" type="reset" id="btnCancel" name="btnCancel" value="Cancel" onclick="window.location.href='<?php echo base_url();?>index.php/users/documents'" />
                                </div>
                        </div>
                    </form>
                </div><!-- End of .content -->
        ...
        <?php 
        /*
         * REMOVED, IT'S FOR OFFICE USE ONLY
         * <div class="grid_12">
            <div class="box">
                <div class="header">
                    <h2><img class="icon" src="<?php echo base_url(); ?>img/icons/packs/fugue/16x16/table.png">Teachers other documents</h2>
                </div>
                <div class="content">
                    <form enctype="multipart/form-data" method="post" class="validate" id="frmTeacher" action="">
                        ...
                        <div class="row">
                                <div class="form-data" style="margin-left: 130px;">
                                    <input class="btn btn-tuition" type="button" id="btnAddFile" name="btnAddFile" value=" + Add File" />
                                    <input class="btn btn-tuition" type="button" id="btnUpload" name="btnUpload" value="Upload" />
                                    <input class="btn btn-tuition" type="reset" id="btnBackReview" name="btnBackReview" value=" << Back" onclick="window.location.href='<?php echo base_url();?>index.php/teachers/<?php echo ($formData["ta_job_adv_sent"] ? "profilereview" : "review" );?>'" />
