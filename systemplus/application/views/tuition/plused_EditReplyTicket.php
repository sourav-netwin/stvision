						...
						<div class="content">
							<form name="replyTicketForm" id="replyTicketForm" method="POST" action="" enctype="multipart/form-data">
								...
								<div class="row form-group">
									...
									<div class="form-data" >
										<textarea name="tktMessage" id="tktMessage" class="form-control required" style="max-width: 100%" maxlength="2000" ><?php echo empty($reply['ptc_bo_reply']) ? '' : $reply['ptc_bo_reply'] ?></textarea>
									</div>	
								</div>	
								<div class="row form-group">
									...
									<div class="form-data" >
										...
										<?php
										if ($reply['ptc_bo_attachment']) {
											?>
											<div style="margin-top: -10px !important;"><a target="_blank" href="<?php echo TICKET_BO_DWNLD . $reply['ptc_bo_attachment'] ?>">Find added attachment</a>&nbsp;&nbsp;<span><a href="javascript:void(0)" style="color: #FF0000" id="removeAttachment" data-id="atmRem_<?php echo $reply['ptc_id'] ?>">Remove attachment</a></span></div>
											<?php
										}
										?>
									</div>	
								...
								<div class="row form-group" style="text-align: center; padding: 10px;">
									<input class="btn btn-primary" id="btnSave" name="btnReplyUpdate" value="Update" type="submit">
		...
		<script type="text/javascript">
			...
			$('#replyTicketForm').submit(function(e){
				...
				if(replyContent == '' || typeof replyContent == 'undefined'){
					...
					$.ajax({
						url: siteUrl+'ticketmanagement/editReplyTicket/<?php echo $ptc_id ?>',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
			...
			$('#removeAttachment').on('click', function(){
				...
				if(c){
					...
					if(selId != '' && typeof selId != 'undefined'){
						$.ajax({
							url: siteUrl+'ticketmanagement/removeReplyAttachment',
							type: 'POST',
							data: {
								selId: selId
							},
