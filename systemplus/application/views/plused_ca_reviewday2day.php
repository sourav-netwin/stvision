...
<body>	
		<section id="content" class="container_12 clearfix" data-sort=true style="padding-left:0px;">
			<div class="grid_12">
				<div class="box">
				
					<div class="header">
						<h2 style="float:left;border:none;"><img class="icon" src="<?php echo base_url(); ?>img/icons/packs/fugue/16x16/table.png">Review campus by month - <?php echo $campusname ?></h2>
						<form name="" action="" method="POST" style="float:right;">
							<select name="accomo" id="accomo" style="margin:12px 12px 0 0;">
								<?php
									foreach($allsis as $asis){
								?>
									<option value="<?php echo strtolower($asis["sistemazione"]) ?>"<?php if($accomodation==strtolower($asis["sistemazione"])){ ?> selected<?php } ?>><?php echo $asis["sistemazione"] ?></option>
								<?
									}
								?>
							</select>
							<select name="month" id="month" style="margin:12px 12px 0 0;">
									<option value="5"<?php if($mese==5){ ?> selected<?php } ?>>May</option>
									<option value="6"<?php if($mese==6){ ?> selected<?php } ?>>June</option>
									<option value="7"<?php if($mese==7){ ?> selected<?php } ?>>July</option>
									<option value="8"<?php if($mese==8){ ?> selected<?php } ?>>August</option>
									<option value="9"<?php if($mese==9){ ?> selected<?php } ?>>September</option>
							</select>
							<input type="submit" name="sendma" id="sendma" value="Send" style="margin-right:12px;padding:2px 8px;" />
						</form>
					</div>
					<div class="content">
						...
						<table class="styled simmy" data-table-tools='{"display":false}'> <!-- OPTIONAL: with-prev-next -->
							...
							?>
								<tr id="riga_<?php echo $contarighe?>">
									<td class="n_<?php echo $book["status"] ?>"><?php echo $book["businessname"] ?><a class="ca_info" id="info_<?php echo $book["id_year"] ?>_<?php echo $book["id_book"] ?>" href="javascript:void(0);"><img class="icon" src="<?php echo base_url(); ?>img/icons/packs/fugue/16x16/information-small.png"></a><input type="hidden" value="<?php echo $book["num_in"] ?>" id="pax_<?php echo $contarighe?>"></td>
								...
								?>									
									<td class="n_<?php echo $book["status"] ?>"><?php echo $book["arrival_date"] ?></td>
									<td class="n_<?php echo $book["status"] ?>"><?php echo $book["departure_date"] ?></td>
								</tr>
							<?php
									$contarighe++;
								}
		...
		<input type="hidden" value="" name="hidYear" id="hidYear" />
		<input type="hidden" value="" name="hidBook" id="hidBook" />
<script>
...
$(document).ready(function(){
	...
	$( ".windia" ).dialog({
			...
			buttons: [{
				...
				click: function() { $(".windia table").printElement(); }
					...
					click: function() {
						var myHidYear = $("#hidYear").val();
						var myHidBook = $("#hidBook").val();
						window.location.href = '<?php echo base_url(); ?>index.php/backoffice/csvCaMyPax/'+myHidYear+'/'+myHidBook;
					}
				}],
			height : 500,
			width: 800
	...
	$(".ca_info").click(function(){
			$("#hidYear").val();
			$("#hidBook").val();		
			var bytd = $(this).attr("id");
			var splitbytd = bytd.split("_");
			var byyear = splitbytd[1];
			var bybook = splitbytd[2];	
			$("#hidYear").val(byyear);
			$("#hidBook").val(bybook);				
			$( "#dialog_modal" ).load('<?php echo base_url(); ?>index.php/backoffice/detCaMyPax/'+byyear+'/'+bybook).dialog("open");
			return false;			
	});	
