...
?>		
		...
		<section id="content" class="container_12 clearfix" data-sort=true>
			<h1 class="grid_12 margin-top no-margin-top-phone">Campus availability day 2 day</h1>
			<div class="row">
				<form style="margin:10px;" id="box_campus" name="box_campus" action="<?php echo base_url(); ?>index.php/backoffice/reviewday2day_pax" method="post">  
					<div class="grid_3">
						<div class="box">
							<div class="header">
								<h2>Select Campus</h2>
						...
						<div class="box">
							...
							<div class="content" style="margin:10px;">
										<select name="accomodation_in" id="accomodation_in">
											...
											<option <?php if($accomodation==$singleSis["tba_id"]){ ?>selected <?php } ?>value="<?php echo $singleSis["tba_id"] ?>"><?php echo $singleSis["tba_type_join"] ?></option>
											<?php } ?>
										</select> 
							</div>
						</div>
						...
						<div class="box">
							...
							<div class="content" style="margin:10px;">
										<select name="year_in" id="year_in">
											<?php
											for($annino=2012;$annino<=date("Y")+1;$annino++){
											?>
			...
			<div class="grid_12">
				<div class="box">
					<div class="header">
						<h2><img class="icon" src="<?php echo base_url(); ?>img/icons/packs/fugue/16x16/calendar.png"><?php echo $campusname?> - <?php echo ucfirst($accomodationname)?> accomodation (<?php echo $monthnames[$month] ?> <?php echo $year ?>)<a  style="float:right;" href="<?php echo base_url(); ?>index.php/backoffice/simulatorday2day/<?php echo $campus;?>/<?php echo $accomodationname;?>/<?php echo $year;?>-<?echo $month?>-01/<?php echo $year;?>-<?echo $month?>-<?php echo count($bkgmese) ?>" title="Simulate month"><img class="icon" src="<?php echo base_url(); ?>img/icons/packs/fugue/16x16/calendar-blue.png">Launch simulation</a></h2>
					</div>
	...
	$(document).ready(function() {
		...
		$( ".windia" ).dialog({
				...
				buttons: [{
					...
					click: function() {
						...
						window.location.href = '<?php echo base_url(); ?>index.php/backoffice/csvArrivalPax_pax/<?php echo $campus ?>/<?php echo $accomodation ?>/'+myHidDate+'/confirmed/'+myTypeForCsv;
					}
				}],
				height : 500,
				width: 800
		...
		$(".openDoorPaxList").click(function(){
				...
				$("#hidDate").val(bydate);
				$("#dialog_modal").load('<?php echo base_url(); ?>index.php/backoffice/infoday2dayArrivalPax_pax/<?php echo $campus ?>/<?php echo $accomodation ?>/'+bydate+'/confirmed/arrival');
				return false;			
		});		
		...
		$(".closeDoorPaxList").click(function(){
				...
				var bydate = splitbytd[1];	
				$("#hidDate").val(bydate);				
				$( "#dialog_modal" ).load('<?php echo base_url(); ?>index.php/backoffice/infoday2dayArrivalPax_pax/<?php echo $campus ?>/<?php echo $accomodation ?>/'+bydate+'/confirmed/departure');
				return false;			
		});	
		...
		$(".allPaxList").click(function(){
				...
				var bydate = splitbytd[1];
				$("#hidDate").val(bydate);				
				$( "#dialog_modal" ).load('<?php echo base_url(); ?>index.php/backoffice/infoday2dayArrivalPax_pax/<?php echo $campus ?>/<?php echo $accomodation ?>/'+bydate+'/confirmed/');
				return false;			
		});			
	});
	</script>	
