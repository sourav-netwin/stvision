...
$(function () {
    $('#containerLondon').highcharts({
        ...
        series: [{
			...
			<?php 
                        if($totCampus)
                        foreach($totCampus as $campus){ 
                                $valore = 0;
                                if($campus[0] != 0 && $campus[4] != 0)
                                    $valore = $campus[0]*100/$campus[4];
				if($valore==0)
					$valore="null";
			?>
			<?php echo round($valore).","; ?>
			<?php } ?>
			],
			stack:0,
			pointPadding: 0
			...
			<?php 
                        ...
                        foreach($totCampus as $campus){ 
                            $valore = 0;
                                if($campus[1] != 0 && $campus[4] != 0)
				$valore = $campus[1]*100/$campus[4];
				if($valore==0)
					$valore="null";
			?>
			<?php echo round($valore).","; ?>
			...
			<?php 
                        ...
                        foreach($totCampus as $campus){ 
                            $valore = 0;
                                if($campus[2] != 0 && $campus[4] != 0)
				$valore = $campus[2]*100/$campus[4];
				if($valore==0)
					$valore="null";
			?>
			<?php echo round($valore).","; ?>
			...
			<?php 
                        ...
                        foreach($totCampus as $campus){ 
                            $valore = 0;
                                if($campus[3] != 0 && $campus[4] != 0)
				$valore = $campus[3]*100/$campus[4];
				if($valore==0)
					$valore="null";
			?>
			<?php echo round($valore).","; ?>
