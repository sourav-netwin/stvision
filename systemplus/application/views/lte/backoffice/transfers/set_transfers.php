    ...
    <div class="col-xs-12">
      <div class="box box-primary">
        <div class="box-header col-sm-12">
          ...
          <form id="box_transport" name="box_transport" action="<?php echo base_url(); ?>index.php/transfers/setTransfers/<?php echo $in_out ?>" method="POST">
        ...
        <div class="box-body">
          ...
          ?>
          <form id="alltran" name="alltran" action="<?php echo base_url(); ?>index.php/transfers/setTransfersTransport/<?php echo $in_out ?>/<?php echo $campus ?>/<?php echo $datagiusta ?>" method="POST">
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <?php if($exc["ptt_buscompany_code"]!=""){ ?>
                            <button <?php if($campus==""){ ?>disabled <?php } ?>class="btn btn-block btn-xs btn-danger addToTransfer" id="<?php echo $exc["ptt_buscompany_code"]?>_<?php echo $exc["id_book"] ?>_<?php echo $exc["id_year"] ?>_<?php echo $exc["totnumpax"]?>" name="">Add to <?php echo $exc["ptt_buscompany_code"]?></button>
                          <?php } ?>
                        </td>
                      </tr>
                <?php
...
$(document).ready(function() {
  ...
  $("#tra_all").click(function(){
    ...
    $(".addToTransfer").click(function(e){
        e.preventDefault();
        var arrmid = $(this).attr("id").split("_");
        confirmAction("Are you sure you want to add these pax to this service?", 
        function(c)
        {
            if(c)
                window.location.replace("<?php echo base_url(); ?>index.php/transfers/addPaxToExistingTransfer/"+arrmid[0]+"/"+arrmid[1]+"/"+arrmid[2]+"/"+arrmid[3]);
        }, true, true);
    });

  $('#findTr').on('click', function(e){
        /*var passingData = $('#box_campus').serialize();
        $.post( "<?php //echo base_url(); ?>index.php/transfers/transferCalendar/?"+passingData, function( data ) {
