				...
				<div id="mpj_2_1" class="tab-pane">
					...
					?>
						...
						<div class="panel-collapse collapse" id="collapse<?echo $cUK["id"] ?>" aria-expanded="false" style="height: 0px;">
                                                <div class="box-body">
                                                    <?php
                                                            // fetch files for campus.
                                                            $campusId = $cUK["id"];
                                                            $campusPdfs = $CI->_getCampusUploadedPdfs($campusId);
                                                        ?>
                                                            <ul>
                                                                <?php
                                                                        if($campusPdfs){
                                                                                foreach($campusPdfs as $cPdfs){
                                                                                        ?>
                                                                                        <li style="text-transform:capitalize;">
                                                                                            <a target="_blank" href="<?php echo base_url() . CAMPUS_AGENTS_SINGLE_PDF_PATH. $cPdfs["pdf_path"]; ?>" title="Download <?php echo htmlentities($cPdfs["title"]); ?>"><?php echo htmlentities($cPdfs["title"]); ?></a>
                                                                                        </li>
                                                                                <?php
                                                                                }
                                                                        }else{?>
...
?>
		...
		?>
            ...
            <div id="mpj_13" class="tab-pane">
                ...
                <div class="col-sm-12">
                    <!-- removed old files listing -->
                    <!-- NEW CODE FOR CAMPUS PRICE LISTING DOCUMENTS -->
                    <ul>
                    <?php if($campusPriceList){?>
                        <?php foreach($campusPriceList as $camFiles){?>
                            <li style="text-transform:capitalize;">
                                <a target="_blank" href="<?php echo base_url() . CAMPUS_PRICELIST_PDF_PATH. $camFiles["pricelist_pdf_path"]; ?>" title="Download "><?php echo $camFiles["pricelist_title"];?></a>
                            </li>
                        <?php }?>
                    <?php }else{?>
                            <li>No documents in this section</li>
		...
		<div id="mpj_14" class="tab-pane">
                    ...
                    <div class="col-sm-12">
                    <!-- removed old files listing -->
                    <!-- NEW CODE FOR TRANSFER PRICE LISTING DOCUMENTS -->
                    <ul>
                    <?php if($transferPriceList){?>
                        <?php foreach($transferPriceList as $camFiles){?>
                            <li style="text-transform:capitalize;">
                                <a target="_blank" href="<?php echo base_url() . CAMPUS_PRICELIST_PDF_PATH. $camFiles["pricelist_pdf_path"]; ?>" title="Download "><?php echo $camFiles["pricelist_title"];?></a>
                            </li>
                        <?php }?>
...
<!--			</div> End of .vertical-tabs
			...
			<ul class="nav nav-tabs">
				<li class="current active">
                                    <a data-toggle="tab" aria-expanded="true" href="#mpj_1"><span class="icon icon-list-alt"></span>Plus Junior</a></li>
				<li><a data-toggle="tab" aria-expanded="true" href="#mpj_2_1" href="javascript:void(0);"><span class="icon icon-list-alt"></span>Destination UK</a></li>
