    ...
    function overviewBookingsNew($campus, $agent, $status, $confirm = 0, $season = 0, $lm = 0, $cfd = 0, $locked = 0) {
        ...
        if ($Q->num_rows() > 0) {
            foreach ($Q->result_array() as $row) {
                ...
                $recordvaluta = $queryvaluta->result_array();
                //print_r($row);
                if ($nomedelcentro != "None" && !empty($recordvaluta)) {
                    //echo $nomedelcentro."<br />";
                    if (isset($recordvaluta[0])) {
                        $row['valuta'] = $recordvaluta[0]['valuta'];
                        $row['valore_acconto'] = $recordvaluta[0]['valore_acconto'];
                    }
                } else {
    ...
    function overviewSingleBooking($idBook) {
        ...
        if ($Q->num_rows() > 0) {
            foreach ($Q->result_array() as $row) {
                ...
                $recordvaluta = $queryvaluta->result_array();
                //print_r($row);
                if ($nomedelcentro != "None" && isset($recordvaluta[0])) {
                    $row['valuta'] = $recordvaluta[0]['valuta'];
                    $row['valore_acconto'] = $recordvaluta[0]['valore_acconto'];
                } else {
                    $row['valuta'] = "$";
...
//function ereditate dal vecchio gestione_contabile_model
    ...
    function getTotBkAsPerPaxRows($campus, $accomodation = "", $date, $multistatus, $glIncluded = 1) {
        $bkok = "";
        //print_r($multistatus);
        if ($accomodation == "") {
    ...
    function getArrBk($campus, $accomodation = "", $date) {
        ...
        $Q = $this->db->query($querya);
        // write_my_log("call mbackoffice getArrBk function");
        if ($Q->num_rows() > 0) {
            foreach ($Q->result_array() as $row) {
                if (!$row["num_in"]) {
                    $arrbk = 0;
...
//	echo $this->db->last_query();
    ...
    function getSimCalendarAllAccos_EachDayCount($campus, $datein, $dateout, $multistatus, $glIncluded = 1) {
        // Set timezone
        date_default_timezone_set('UTC');
        // Start date
        $date = $datein;
        ...
        $contagiri = 0;
        while (strtotime($date) <= strtotime($end_date)) {
            $backcal[] = $this->getTotAvaAllAccos_EachDayCount($campus, $date, $multistatus, $glIncluded);
            $date = date("Y-m-d", strtotime("+1 day", strtotime($date)));
            $contagiri++;
        }
        return $backcal;
    ...
    function getTotAvaAllAccos_EachDayCount($campus, $date, $multistatus, $glIncluded = 1) {
        // CHANGES MADE: 14-July-2017 :SK
        // CHANGED function getTotBk to getTotBkAsPerPaxRows
        ...
        if ($Q->num_rows() > 0) {
            foreach ($Q->result_array() as $row) {
                ...
                //$multistatus = array("confirmed","active");
                $booked = $this->getTotBkAsPerPaxRows($campus, "", $date, $multistatus, $glIncluded);
                $avarest = $avaok - $booked;
                if ($avarest >= 0) {
                    $record["avaOver"] = "r_available";
                } else {
                ...
                $record["n_available"] = $avarest;
                $multistatus = array("confirmed");
                $confirmed = $this->getTotBkAsPerPaxRows($campus, "", $date, $multistatus, $glIncluded);
                $record["n_confirmed"] = $confirmed;
                $multistatus = array("active");
                $active = $this->getTotBkAsPerPaxRows($campus, "", $date, $multistatus, $glIncluded);
                $record["n_active"] = $active;
                $multistatus = array("tbc");
                $tbc = $this->getTotBkAsPerPaxRows($campus, "", $date, $multistatus, $glIncluded);
                $record["n_tbc"] = $tbc;
                $multistatus = array("elapsed");
                $elapsed = $this->getTotBkAsPerPaxRows($campus, "", $date, $multistatus, $glIncluded);
                $record["n_elapsed"] = $elapsed;
                $record["datat"] = $date;
            }
        }
    ...
    function NA_getSimBookingAllAccos_backoffice_EachDayCount($campus, $datein, $dateout, $arrayStatus, $includedGL = 1) {
        // Set timezone
        date_default_timezone_set('UTC');
        // Start date
        $date = $datein;
        ...
        $dateFilters = "plused_rows.data_partenza_campus >= '$datein' AND plused_rows.data_arrivo_campus <= '$dateout'";
        $querya = "SELECT DISTINCT plused_book.id_book, plused_book.id_year, agenti.businessname, 
            COUNT(plused_book.id_book) as num_in, plused_rows.data_arrivo_campus, plused_rows.data_partenza_campus, 
            plused_book.status, agenti.businesscountry FROM plused_book, plused_rows, agenti 
        ...
        if ($Q->num_rows() > 0) {
            foreach ($Q->result_array() as $row) {
                $currBookRow = array();
                $dateLoop = $date;
                while (strtotime($dateLoop) <= strtotime($end_date)) {
                    $currBookRow[$dateLoop] = $this->getBokingsOnDay($row['id_book'],$dateLoop);
                    $dateLoop = date ("Y-m-d", strtotime("+1 day", strtotime($dateLoop)));
                }
                $row['bookings_on_days'] = $currBookRow;
                $backbkg[] = $row;
            }
        }
        $Q->free_result();
    ...
    function getBokingsOnDay($bookId,$date){
        $this->db->where('id_book',$bookId);
        $this->db->where('tipo_pax <> ',"GL");
        $this->db->where('data_arrivo_campus <= ',$date);
        $this->db->where('data_partenza_campus >= ',$date);
        $result = $this->db->get('plused_rows');
        return $result->num_rows();
    }
    
    /**
     * This function is used in backofficecrons controller set cron job
     * @return type
     */
    function elapsedBookingsToElapse() {
        ...
        if ($resultData->num_rows()) {
            foreach ($resultData->result_array() as $booking) {
                ...
                // OLD QUERY: $queryUpd = "UPDATE plused_book SET status = 'elapsed' WHERE data_scadenza < '" . $dataOggi . "' AND status = 'active' AND acconto_versato = 0";
                $updateData = array(
                    'status' => 'elapsed'
                );
                $this->db->flush_cache();
                $this->db->where('id_book', $booking['id_book']);
        ...
        if ($Q->num_rows() > 0) {
            foreach ($Q->result_array() as $row) {
                ...
                if ($QNotes->num_rows() > 0) {
                    ...
                    if(!isset($row["pbe_cm_done"]))
                        $row["pbe_cm_done"] = "";
                    if(!isset($row["pbe_cm_ok"]))
                        $row["pbe_cm_ok"] = "";
                    if(!isset($row["pbe_cm_notes"]))
                        $row["pbe_cm_notes"] = "";
                }
                $QNotes->free_result();
                $data[] = $row;
...
//NEW FUNCTIONS PER REVIEWDAY2DAY - TERMINATI I TEST RIMUOVERE TUTTE LE FUNCTIONS SENZA SUFFISSO _pax, IDEM DAL CONTROLLER E IDEM PER LE VIEW
    ...
    function ca_getExcursionsBusCodesForDay($date, $campus) {
        $datatra = array();
        $querya = "SELECT exc_excursion, exc_length, exb_type, exb_buscompany_code, sum(exb_tot_pax) as exb_tot_pax, exb_excursion_date, exb_confirmed FROM plused_exc_bookings, plused_exc_all WHERE exb_id_excursion = exc_id AND exb_campus_id = $campus AND exb_excursion_date = '$date' AND (exb_confirmed = 'YES' OR exb_confirmed = 'STANDBY') GROUP BY exb_buscompany_code";
        $Q = $this->db->query($querya);
        if ($Q->num_rows() > 0) {
            foreach ($Q->result_array() as $row) {
                $datatra[] = $row;
    ...
    function setUnplannedAllExcursions($campus, $tipo) {
        ...
        if ($campus) {
            ...
            //$query1 = "SELECT pte_id, exc_id, exc_excursion, nome_centri, pte_tot_pax, pte_book_id, exc_length, pte_campus_id, pte_fromCampusManagerTick, pte_fromCampusManagerAmount, pte_proforma_currency FROM plused_tra_excursions, plused_exc_all, centri WHERE exc_id = pte_excursion_id and centri.id = pte_campus_id  ORDER BY exc_id, pte_book_id";
            ...
            if ($Q->num_rows() > 0) {
                foreach ($Q->result_array() as $row) {
                    ...
                    $queryfill = "SELECT plused_book.arrival_date, plused_book.departure_date, plused_book.status as statopre, agenti.businessname FROM plused_book, agenti WHERE  plused_book.id_agente = agenti.id";
    ...
    function cmsAddCampus() {
        ...
        if($idC)
        {
            //plused_join_prodotti_centri
            $addRow = array(
                'pjpc_centro' => $idC,
                'pjpc_prodotto' => 1
            );
            $this->db->insert('plused_join_prodotti_centri', $addRow);
    ...
    function updateCampusStatus($idC,$status){
        $data = array(
            'attivo' => $status
        );
        $this->db->where('id', $idC);
    ...
    function updateCampusImage($idC,$imageWeb){
        $data = array(
            'website_image' => $imageWeb
        );
        $this->db->where('id', $idC);
        $this->db->update('centri', $data);
        return 1;
    ...
    function getGlSTDCount($idb) {
        $data = array();
        //MODIFICARE SE VENGONO AGGIUNTI TIPI PAX (ADT)
        $sqlid = "SELECT COUNT(tipo_pax) as contot,COUNT(IF(tipo_pax='GL',1,NULL)) AS contoGL, COUNT(IF(tipo_pax='STD',1,NULL)) AS contoSTD FROM plused_rows where id_book = $idb";
        $query = $this->db->query($sqlid);
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rowq) {
                $data[] = $rowq;
...
//new transfers functions
    ...
    function getBookingRefFromCentro($campusId = 0) {
        $this->db->select('CONCAT(id_year, \'_\', id_book) as booking_id', false)
                ->from('plused_book')
                ->where('id_year', date('Y'));
        if (is_array($campusId)) {
            $this->db->where_in('id_centro', $campusId);
        } else if (is_numeric($campusId) && $campusId > 0) {
    ...
    function getTicketDetails($ptcId = '', $campus = '', $type = '', $sent = 1) {
        if ($ptcId) {
            $this->db->where('ptc_id', $ptcId);
        }
        if ($sent == 0) {
            if ($type) {
                $this->db->where('ptc_receiver_type', $type);
            }
        } else {
            if ($type) {
                $this->db->where('ptc_sender_type', $type);
            }
        }
    ...
    function getOpenTicketDetails($limit = '20') {
        $this->db->select('a.*, DATEDIFF(\'' . date('Y-m-d') . '\', a.ptc_created_time) as dateDf, c.nome_centri ', false)
                ...
                ->where('a.ptc_receiver_type', 'Backoffice')
    ...
    function cmsInsertCampusImage($campus_id, $title, $category, $filename) {
        $data = array(
            'campus_id' => $campus_id,
            'title' => $title,
            'category' => $category,
    ...
    function cmsInsertCampusSinglePdfs($campus_id, $title, $filename) {
        $data = array(
            'campus_id' => $campus_id,
            'title' => $title,
            'pdf_path' => $filename
        );
        $this->db->insert('plused_campus_single_pdf_for_agents', $data);
        return $this->db->insert_id();
    }
    
    function getCampusSinglePdfForAgent($campus_pdf_id) {
        $data = array();
    ...
    function getCampusSinglePdfsForAgent($campus_id) {
        $data = array();
        $this->db->select("*");
        $this->db->from("plused_campus_single_pdf_for_agents");
    ...
    function updatePdfsForAgentSequence($pdf_id,$updateArr,$campus_id){
        $this->db->where("campus_single_pdf_id", $pdf_id);
        $this->db->update('plused_campus_single_pdf_for_agents',$updateArr);
        return 1;
    }
    ...
    function getCampusVideo($campus_id) {
        $data = array();
        $this->db->select("*");
        $this->db->from("plused_campus_video");
        $this->db->where("campus_id", $campus_id);
        $this->db->limit(1);
        $query = $this->db->get();
    ...
    function cmsInsertCampusVideo($campus_id, $insert_data) {
        // Check if record exists
        $this->db->select("campus_video_id");
        $this->db->from("plused_campus_video");
        $this->db->where("campus_id", $campus_id);
        $this->db->limit(1);
        $query = $this->db->get();
        $id = ( $query->num_rows() > 0 ) ? $query->row()->campus_video_id : 0;

        if ($id == 0) {
            if (!( $insert_data['campus_video_1'] == '' && $insert_data['campus_video_2'] == '' && $insert_data['campus_video_3'] == '' && $insert_data['campus_video_4'] == '' )) {
                $insert_data['campus_id'] = $campus_id;
                $this->db->insert('plused_campus_video', $insert_data);
            } else
                $id = '-';
        }
        else {
            if ($insert_data['campus_video_1'] == '' && $insert_data['campus_video_2'] == '' && $insert_data['campus_video_3'] == '' && $insert_data['campus_video_4'] == '') {
                $this->db->where('campus_video_id', $id);
                $this->db->delete('plused_campus_video');
            } else {
                $this->db->where('campus_video_id', $id);
                $this->db->update('plused_campus_video', $insert_data);
            }
        }
        return $id;
    }

    function editBookingDate($date_in, $date_out, $booking_id) {
        $da = explode('/', $date_in);
        $dd = explode('/', $date_out);

        $date_in = $da[1] . '/' . $da[0] . '/' . $da[2];
        $date_out = $dd[1] . '/' . $dd[0] . '/' . $dd[2];

        $data_book = array(
            'arrival_date' => date('Y-m-d', strtotime($date_in)),
            'departure_date' => date('Y-m-d', strtotime($date_out))
        );
        $this->db->where('id_book', $booking_id);
        $this->db->update('plused_book', $data_book);

        $data_rows = array(
            'andata_data_arrivo' => date('Y-m-d H:i:s', strtotime($date_in)),
            'ritorno_data_partenza' => date('Y-m-d H:i:s', strtotime($date_out)),
            'data_arrivo_campus' => date('Y-m-d H:i:s', strtotime($date_in)),
            'data_partenza_campus' => date('Y-m-d H:i:s', strtotime($date_out))
        );
        $this->db->where('id_book', $booking_id);
        $this->db->update('plused_rows', $data_rows);

        return ( $this->db->affected_rows() > 0) ? TRUE : FALSE;
    }

    function get_total_available($campus, $accomodation, $date) {
        $record = array();
        $where_arr = array(
            'start_date <=' => $date,
            'finish_date >=' => $date,
        ...
        $this->db->select("SUM(availability) as dispo");
        $this->db->from("plused_campus_availability");
        $this->db->where($where_arr);
        $this->db->limit(1);
        $query = $this->db->get();

        if ($query->num_rows() > 0) {
            $record["totale"] = (!$query->row()->dispo ) ? 0 : $query->row()->dispo;
        }
        return $record;
    }
    ...
    function migrateExcursions() {
        $resultMessage = "";
        $countRecordAdded = 0;
        $this->db->select("exc_id,exc_id_centro,exc_centro,exc_length,exc_excursion,exc_type,exc_days,exc_weeks,exc_airport,exc_transfer_type");
        $result = $this->db->get("plused_exc_all");

        if ($result->num_rows() > 0) {
            foreach ($result->result_array() as $row) {
                // CHECK EXCURSION ID ALREADY ADDED OR NOT
                $alreadyFound = false;
                $this->db->flush_cache();
                $this->db->where('exc_old_exc_id', $row['exc_id']);
                $existingRec = $this->db->get('agnt_excursions');
                if ($existingRec->num_rows()) {
                    $alreadyFound = true;
                }
                if (!$alreadyFound) {
                    // INSERT EXCURSION MASTER RECORD....
                    $insertArray = array(
                        'exc_excursion_name' => $row['exc_excursion'],
                        'exc_brief_description' => $row['exc_centro'],
                        'exc_type' => $row['exc_type'],
                        'exc_days' => $row['exc_days'],
                        'exc_weeks' => $row['exc_weeks'],
                        'exc_airport' => $row['exc_airport'],
                        'exc_old_exc_id' => $row['exc_id']
                    );
                    $insertRes = $this->db->insert("agnt_excursions", $insertArray);
                    if ($insertRes) {
                        $insertId = $this->db->insert_id();
                        $countRecordAdded++;
                        // ADD TO EXCURSION CAMPUS
                        $excursionCampus = array(
                            'excm_campus_id' => $row['exc_id_centro'],
                            'excm_exc_id' => $insertId
                        );
                        $this->db->insert("agnt_campus_excursion", $excursionCampus);
                    }
                }
            }
        }
        $resultMessage = "Total number of excursions migrated " . $countRecordAdded;
        echo $resultMessage;
    }

    function getCampusIdFromName($campusName, $addNewCampus = FALSE) {
        $this->db->flush_cache();
        $this->db->select('id')
                ->from('centri')
                //-> where('attivo', 1)
                ->where('nome_centri', trim($campusName));
        $result = $this->db->get();
        if ($result->num_rows() > 0) {
            ...
            if ($addNewCampus) {
                $insertCampus = array(
                    'nome_centri' => $campusName
                );
                $this->db->insert('centri', $insertCampus);
                return $this->db->insert_id();
            } else
                return 0;
        }
    ...
    function insertExcursion($excursionInsert) {
        $this->db->insert('plused_exc_all', $excursionInsert);
        return $this->db->insert_id();
    }

    public function br_getBookingsCountForBursarReview($campusId, $status, $dateSel, $month, $year) {
        date_default_timezone_set('UTC');
        // Start date
        $date = $year . '-' . $month . '-01';
        // End date
        ...
        $dateout = date('Y-m-d', strtotime($dateinA . "+15 days"));
        while (strtotime($datein) <= strtotime($dateout)) {
            $backjson[] = $this->ca_getCountBookings($campusId, $datein, $status);
            $datein = date("Y-m-d", strtotime("+1 day", strtotime($datein)));
        }
        return $backjson;
    }

    public function ca_getCountBookings($campus, $date, $status) {
        $query = $this->db->from('plused_book');
        $this->db->join('plused_rows', 'plused_rows.id_book=plused_book.id_book');
        $this->db->where('plused_rows.id_year = plused_book.id_year');
        $this->db->where('plused_book.status', $status);
        $this->db->where('plused_book.id_centro', $campus);
        $this->db->where('plused_rows.data_arrivo_campus <=', $date . ' 23:59');
        $this->db->where('plused_rows.data_partenza_campus >', $date . ' 23:59');
        return $this->db->count_all_results();
    }

    public function getActiveBookingsForBursars($campus, $date) {
        $this->db->select('plused_book.id_book,plused_book.id_year,count(plused_rows.id_prenotazione) as pax_totali, plused_book.status, centri.nome_centri as centro, plused_book.arrival_date, plused_book.departure_date');
        $this->db->from('plused_book');
        $this->db->join('plused_rows', 'plused_rows.id_book=plused_book.id_book');
        ...
        $this->db->where('plused_book.status', 'active');
        $this->db->where('plused_book.id_centro', $campus);
        $this->db->where('plused_book.arrival_date <=', $date);
        $this->db->where('plused_book.departure_date >=', $date);
        $this->db->group_by('plused_rows.id_book');
        $result = $this->db->get();
        return $result->result_array();
    }

    public function getTicketUserForEmail($campuses, $userTypes) {
        $this->db->_reset_select();
        $this->db->select('email,role,first_name');
        $this->db->from('members');
        $this->db->where_in('campusid_ref', $campuses);
        $where = '';
        if (in_array('Campus Manager', $userTypes) && in_array('Course Director', $userTypes)) {
            $where = '(role="college_adm" OR role="course_director")';
        } elseif (in_array('Campus Manager', $userTypes)) {
            $where = '(role="college_adm")';
        } else if (in_array('Course Director', $userTypes)) {
            $where = '(role="course_director")';
        }
        if (!empty($where)) {
            $this->db->where($where);
        }
    ...
    public function getTicketReceiver($id, $role) {
        $this->db->select('email,ptc_title');
        $this->db->from('members');
        $this->db->join('plused_ticket_cm', 'plused_ticket_cm.campus_id=members.campusid_ref');
        $this->db->where('role', $role);
        $this->db->where('plused_ticket_cm.ptc_id', $id);
        $result = $this->db->get();
        return $result->result_array();
    }
