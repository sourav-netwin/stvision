	...
	class Master_activity extends Controller
	{
		...
		*/
		public function add_edit($id = NULL)
		{
			...
			if($id != '')
				$post = $this->master_activity_model->getEditdata($id);
			$data['post'] = $post;
			$data['id'] = $id;
			$data['flag'] = ($id != '') ? 'es' : 'as';
			$data['breadcrumb1'] = 'Website managements';
		...
		public function add_master_activity()
		{
			if($this->input->post('flag'))
			{
				...
				foreach($studentGroup as $groupValue)
				{
					...
					$masterActivityIdArr[] = $masterActivityId;
				...
				$data['datesArr'] = $this->master_activity_model->getDates($masterActivityIdArr);
				echo json_encode($data);
			}
		}
			...
			if($this->input->post('flag'))
			{
				...
				foreach($parentIdArr as $idValue)
				{
					$insertData = array(
						...
						'fixed_day_activity_id' => $idValue
					);
					if($this->input->post('flag') == 'as')
						$activityDetailsIdArr[] = $this->admin_model->commonAdd(TABLE_FIXED_DAY_ACTIVITY_DETAILS , $insertData);
					else
						$this->admin_model->commonUpdate(TABLE_FIXED_DAY_ACTIVITY_DETAILS , "fixed_day_activity_id = ".$idValue." AND fixed_day_activity_details_id in (".$this->input->post('activityDetailsId').")" , $insertData);
				}
		...
		*/
		public function delete_activity_details()
		{
			...
			{
				$this->admin_model->commonDelete(TABLE_FIXED_DAY_ACTIVITY_DETAILS , 'fixed_day_activity_details_id in ('.$this->input->post('id').')');
				echo '';
			}
