...
var starsPath = RATY_BASE_PATH + 'js/raty/img/';
...
(function($) {
		...
		if (!opt.readOnly) {
			...
			if (opt.cancel) {
				...
				$('#' + id + ' img.button-cancel').mouseenter(function() {
					...
					opt.starOffWidth > 0 ? stars.attr('width', opt.starOffWidth) : '';
			...
			if(opt.showOne){
				if (i == score) {
					if (opt.iconRange && opt.iconRange.length > item) {
						starOn = opt.iconRange[(i-1)][0];
						range = i;//opt.iconRange[item][1];
						if (i <= range) {
							star.attr('src', opt.path + starOn);
							opt.starOnWidth > 0 ? star.attr('width', opt.starOnWidth) : '';
						}
					...
					opt.starOffWidth > 0 ? star.attr('width', opt.starOffWidth) : '';
				}
			}
			else{
				if (i <= score) {
	...
	function initialize(context, score, opt) {
		...
		if (isNaN(parseInt(score))) {
			...
			opt.starOffWidth > 0 ? context.children('img.' + id).attr('width', opt.starOffWidth) : '';
			$('input#' + id + '-score').removeAttr('value');
			return;
		}
